<x-admin.app title="Azzahra Decoration - Promotions">
    ...
    <x-admin.form action="{{ route('admin.master-data.promotions.update', $promotion->id) }}" method="POST">
        ...
        <x-admin.input label="Code" type="text" name="code" value="{{ old('code', $promotion->code) }}" />
        <x-admin.input label="Name" type="text" name="name" value="{{ old('name', $promotion->name) }}" />
        <x-admin.select label="Type" name="type" value="{{ old('type', $promotion->type) }}">
        ...
        <x-admin.input label="Discount" type="number" name="discount" value="{{ old('discount', $promotion->discount) }}" step="0.01" />
